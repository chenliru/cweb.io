#!/bin/ksh93
#######################################################################
# Uploading CODA data files to AWS
#----------------------------------------------------------------------
# 
# Author : Claudio
...
then
	...
	mail -s "ICC incremental data upload completed" "$MAILNOTICE" < /dev/null
fi
...
then
	...
	mail -s "ICC incremental data token upload completed" "$MAILNOTICE" < /dev/null	
fi
...
then
	...
	mail -s "ICC incremental Key files upload completed" "$MAILNOTICE" < /dev/null
fi
...
then
	...
	mail -s "ICC incremental Key files token upload completed" "$MAILNOTICE" < /dev/null
fi
