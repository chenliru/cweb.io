...
{*  (c) Copyright IBM Corporation 1996, 2009 All rights reserved.         *}
{*                                                                        *}
{**************************************************************************}

{*********************************************************
 *
 * This sql file should be run against sysadmin database
 * to cleanup and reinstall the AUS feature
 *
 ********************************************************}
...
{*********************************************************
 * Allow us to delete these tasks and disable them so they do
 *   not try and run while we are doing maintenance
 ********************************************************}
UPDATE ph_task SET(tk_attributes,tk_enable)=(0,'f') WHERE
tk_name MATCHES "Auto Update Statistics*";


{*********************************************************
 * Drop all AUS function/SPL
 ********************************************************}
DROP FUNCTION aus_get_realtime();
DROP FUNCTION aus_refresh_stats(INTEGER, INTEGER, INTEGER);
DROP FUNCTION aus_refresh_stats_orig();
DROP FUNCTION aus_refresh_stats_orig(INTEGER, INTEGER);
DROP FUNCTION aus_refresh_upgrade();
DROP FUNCTION aus_refresh_downgrade();
DROP FUNCTION aus_setup_mon_table_profile(INTEGER, INTEGER);
DROP FUNCTION aus_setup_mon_table_profile(INTEGER, INTEGER, INTEGER);
DROP FUNCTION aus_cleanup_table(INTEGER);
DROP FUNCTION aus_setup_table();
DROP FUNCTION aus_setup_table(INTEGER);
...
DROP FUNCTION aus_evaluator_dbs(char(128),INTEGER, INTEGER, INTEGER);
DROP FUNCTION aus_load_dbs_data(char(128),INTEGER);
DROP FUNCTION aus_load_dbs_data(char(128),INTEGER,INTEGER);
...
DROP TABLE aus_cmd_list;
DROP TABLE aus_cmd_comp;


{*********************************************************
 * Cleanup all configuration params
 ********************************************************}
...
{*********************************************************
 * Remove the task from the database scheduler
 ********************************************************}
...
{* The following will reinstall AUS in sysadmin *}
